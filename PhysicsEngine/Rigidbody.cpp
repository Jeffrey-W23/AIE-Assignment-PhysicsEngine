...
void Rigidbody::ResolveCollision(Rigidbody* pActor)
{
	...
	glm::vec3 v3Normal = glm::normalize(pActor->GetPosition() - m_v3Position);
	glm::vec3 v3RelativeVelocity = pActor->GetVelocity() - m_v3Velocity;
	float fElasticity = (m_fElasticity + pActor->GetElasticity()) * 0.5f;

	// Calculate the j formula
	float fJFormula = glm::dot(-(1 + fElasticity) * (v3RelativeVelocity), v3Normal) / glm::dot(v3Normal, v3Normal * ((1 / m_fMass) + (1 / pActor->GetMass())));

	// Calculate force.
	glm::vec3 v3Force = v3Normal * fJFormula;

	// Apply the force to the actor.
	ApplyForceToActor(pActor, v3Force);
}
